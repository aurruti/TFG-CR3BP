...
L5_orbits = np.loadtxt('data/L5_PlanarOrbits.txt')
s0_L5 = np.delete(L5_orbits[-1,:],4,0) # largest
...
maximum_y = [-10,-10,-10,-10]
s_transfer = [0,0,0,0]
h_transfer = 0
for i in range(1,500,5):
    h = i*10**(-7)
    s_new = s0 + h*unstable_dir
    s_end, t_end = shoot_to_poincare_x(mu_earthsun, s_new, style=None, crossings=1, x_obj=L5)
...
print("Manifold's closest approach to L5 at y: " + str(maximum_y[1]))
...
def to_minimize(halfway_point, style_1='none',style_2='none',show=False, step=0.05):
    s_halfway, t_half = shoot_to_poincare_x(mu_earthsun, s_transfer, style=style_1, crossings=1, x_obj=(L5+halfway_point), bstep=step)
    ...
    while not(success) and count<11: # Retry until solution converges
        ...
        ds_guess = root1.x
    s_fin, t_1 = shoot_to_poincare_y(mu_earthsun, [s0_halfway[0],s0_halfway[1],root1.x[0],root1.x[1]], style=style_2, crossings=2, bstep=step, y_obj=s_obj[1])
    #if show: print(s_fin)
    ...
    deltav2 = np.array([s_fin[2],s_fin[3]]) - np.array([s0_L5[2],s0_L5[3]])
    if show: print('Second delta-v (L5):           ' + str(deltav2) + ' [dx,dy] (absolute: ' + str(np.linalg.norm(deltav2)) + ')')
...
L5plusX = optimize.minimize_scalar(to_minimize, 0.2, bounds=(0.1,0.4),method='bounded',args=('none','none',False,0.02),options={'maxiter':7})

print('Optimal burn point found at x: ' + str(L5+L5plusX.x))

totalDv = to_minimize(L5plusX.x,style_1='C1',style_2='C2',show=True)
